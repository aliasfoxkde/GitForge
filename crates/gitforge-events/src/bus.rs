...
pub struct EventFilter {
    ...
    pub repo_id: Option<gitforge_common::RepoId>,
}
...
impl EventFilter {
    ...
    pub fn for_repo(repo_id: gitforge_common::RepoId) -> Self {
        Self {
            event_types: None,
...
mod tests {
    ...
    fn test_event_filter_for_types_no_match() {
        ...
        let event = EventEnvelope::new(
            ...
            EventPayload::JobQueued(crate::event::JobQueuedPayload {
                ...
                pipeline_run_id: gitforge_common::PipelineRunId::new(),
    ...
    fn test_event_filter_for_repo_no_match() {
        let filter = EventFilter::for_repo(gitforge_common::RepoId::new());
        let event = EventEnvelope::new(
            EventType::PushReceived,
            EventPayload::PushReceived(PushReceivedPayload {
                repo_id: gitforge_common::RepoId::new(),
    ...
    fn test_event_filter_matches_with_no_repo_filter() {
        ...
        let event = EventEnvelope::new(
            ...
            Some(gitforge_common::RepoId::new()),
            None,
        );
        assert!(filter.matches(&event));
    }
    ...
    fn test_event_filter_no_match_when_repo_differs() {
        let filter = EventFilter {
            event_types: None,
            repo_id: Some(gitforge_common::RepoId::new()),
        };

        let event = EventEnvelope::new(
            EventType::PushReceived,
            EventPayload::PushReceived(PushReceivedPayload {
                repo_id: gitforge_common::RepoId::new(), // Different repo
                ref_name: "refs/heads/main".to_string(),
                old_hash: "abc".to_string(),
                new_hash: "def".to_string(),
                pusher_id: None,
            }),
            Some(gitforge_common::RepoId::new()), // Different repo
            None,
        );
        assert!(!filter.matches(&event));
    }
