...
```bash
# Review staged changes
gitforge review --staged --provider anthropic

# Review uncommitted changes
gitforge review --diff --provider openai --verbose

# Use local Ollama
gitforge review --staged --provider ollama
```
...
The `ai-review.yml` workflow runs automatically on PRs and posts review comments.

### Security Scanning

The scanner detects 15+ vulnerability types:
- SQL Injection
- Command Injection
